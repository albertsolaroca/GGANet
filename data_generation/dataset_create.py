...
for i, network in enumerate(ordered_networks):
    ...
    for ix_trials, trials in enumerate(n_trials):
        print(f'Working with {network}, network {i+1} of {len(ordered_networks)} || size: {trials}, {ix_trials+1} of {len(n_trials)}')
        try:
            create_and_save(network, path, n_trials=trials, d_attr=d_attr, d_netw=d_netw, out_path=out_path, max_fails=10*trials, show=True)
        except:
