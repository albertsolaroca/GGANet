    ...
    def display_results(self):
        ...
        def response(change):
            ...
            if property == 'Wet R2':
                ...
                sizeref = 2. * max(inverted_r2) / (6 ** 2)
                self.f.update_traces(
                    marker=dict(color=wet_r2, size=inverted_wet_r2, sizeref=sizeref, sizemin=3, colorscale='RdBu'),
                    selector=dict(name="coordinates"))
                self.f.update_traces(text=self._get_text(wet_r2), selector=dict(name="coordinates"))

            if property == 'Dry R2':

                sizeref = 2. * max(inverted_r2) / (6 ** 2)
                self.f.update_traces(
                    marker=dict(color=dry_r2, size=inverted_dry_r2, sizeref=sizeref, sizemin=3, colorscale='RdBu'),
                    selector=dict(name="coordinates"))
                self.f.update_traces(text=self._get_text(dry_r2), selector=dict(name="coordinates"))
